...
public class PathwayEditor : Editor
{
	...
	PathwayHandles _pathwayHandles;
	PathWayNavMeshUI _pathWayNavMeshUI;

	enum LIST_MODIFICATION { ADD, SUPP, DRAG, OTHER };

	LIST_MODIFICATION _currentListModification;
	int _indexCurrentModification;
	...
	void AddItem(ReorderableList list)
	{
		...
		else
		{
			...
			list.serializedProperty.GetArrayElementAtIndex(list.serializedProperty.arraySize - 1).FindPropertyRelative("waypoint").vector3Value =
				new Vector3(previous.x + 2, previous.y, previous.z + 2);
