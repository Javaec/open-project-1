...
{
	[SerializeField] FadeChannelSO _fadeChannelSO;
	[SerializeField] Image _imageComponent;

	bool _isCurrentlyFading = false;
	...
	/// </summary>
	IEnumerator FadeCoroutine(bool fadeIn, float duration, Color endColor = default)
	{
	...
	void InitiateFade(bool fadeIn, float duration, Color desiredColor)
	{
		if (!_isCurrentlyFading
		) // Makes sure multiple fade-ins or outs don't happen at the same time. Note this will mean fadeouts called at the same time will be discarded.
		{
